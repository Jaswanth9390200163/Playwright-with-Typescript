...
- Name: api\mock_from_har.spec.ts >> Mock from HAR File
- Location: tests\api\mock_from_har.spec.ts:5:5
...
Locator: getByText('PlaywrightHAR')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByText('PlaywrightHAR')
...
```ts
  ...
  2  | import { uptime } from "node:process";
  3  | 
  4  | 
  5  | test("Mock from HAR File", async({page})=> {
  6  |     
  7  |     //Record HAR file
  8  |     page.routeFromHAR('.har/fruits.har', {
  9  | 
  10 |         url : '*/**/api/v1/fruits',
  11 |         update : true
  12 |     });
  13 | 
  14 | 
  15 |     await page.goto("https://demo.playwright.dev/api-mocking");
  16 | 
  17 |     await expect(page.getByText('Strawberry')).toBeVisible();
> 18 |     await expect(page.getByText('PlaywrightHAR')).toBeVisible();
     |                                                   ^ Error: expect(locator).toBeVisible() failed
  19 |     await expect(page.getByText('JaswanthHAR')).toBeVisible();
  20 |     await expect(page.getByText('HAR')).toBeVisible();
  21 | 
  22 | 
  23 | });
```