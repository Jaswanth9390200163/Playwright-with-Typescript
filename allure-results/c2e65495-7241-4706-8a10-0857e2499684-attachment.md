...
Locator: getByText('Playwright API Mocking 1')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByText('Playwright API Mocking 1')
...
```ts
  ...
  24 |     //validate text
  25 | 
> 26 |     await expect(page.getByText('Playwright API Mocking 1')).toBeVisible();
     |                                                              ^ Error: expect(locator).toBeVisible() failed
  27 |     
  28 |     await expect(page.getByText('Playwright API Mocking 2')).toBeVisible();
  29 | 
  30 |     await expect(page.getByText('Playwright API Mocking 3')).toBeVisible();
  31 | 
  32 | 
  33 | });
```