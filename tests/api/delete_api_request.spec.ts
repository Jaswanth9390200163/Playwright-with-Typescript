...
test("PATCH api request", async({request})=> {
    ...
    //PATCH api request which requires this token
    ...
    await expect(patchAPIResponse.status()).toBe(200);

    await expect(patchAPIResponse.statusText()).toBe('OK');

    const patchAPIResponseJSON = await patchAPIResponse.json();

    console.log("patch api response is :", JSON.stringify(patchAPIResponseJSON, null, 2));

    expect(patchAPIResponseJSON.firstname).toBe('Typescript');
    ...
    console.log("delete api response body : ", deleteAPIResponse.body());
